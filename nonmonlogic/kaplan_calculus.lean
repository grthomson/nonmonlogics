...
A derivably closed base relation satisfies `IsBaseAdmissible`
automatically for any cut and any proof tree, making all admissible
cuts base-admissible unconditionally.
-/
...
/--
If the base relation is derivably closed, then every cut is
automatically base-admissible.
-/
theorem derivablyClosed_isBaseAdmissible
    {base : BaseRel} (hclosed : IsDerivablyClosed base)
    (t : PTree) (cut : List Address) :
    IsBaseAdmissible base t cut := by
...
def restrictCut (cut : List Address) (i : Nat) : List Address :=
  cut.filterMap (fun addr =>
    match addr with
    | [] => none
    | j :: rest => if h : j = i then some rest else none)
...
Note: the statement uses `cs[i]` directly rather than membership,
because addresses select children by index, not by arbitrary membership.
-/
theorem isBaseAdmissible_restrictCut
  ...
  apply hbase (i :: addr)
  · simp [restrictCut, List.mem_filterMap] at haddr
    rcases haddr with ⟨a, ha, hm⟩
    cases a with
    | nil => simp at hm
    | cons j rest =>
        by_cases hji : j = i
...
This is the key lemma connecting the inductive hypothesis
(which applies `remainderGo` with restricted cut at `[]`)
to the recursive call in the parent
(which applies `remainderGo` with original cut at `[i]`).
-/
...
theorem remainderGo_toTree_is_toTree
  ...
  · have hb : base s.lhs s.rhs := by
      have hsubt : subtreeAt (NMMS.toTree d) [] =
          some (NMMS.toTree d) := by simp [subtreeAt]
  ...
  · cases d with
    ...
    | imp_l d₁ d₂ =>
        ...
        simp only [NMMS.toTree, remainderGo, hroot, ite_false,
                   List.nil_append]
        congr 1
...
/--
Every component appearing in the Connes-Kreimer coproduct of a proof
tree `toTree d` corresponds to a genuine subderivation of `d`,
provided the cut is base-admissible.
-/
