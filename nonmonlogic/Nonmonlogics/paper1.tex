...
\]
for all proof trees $x,y,z$, making $(V,\triangleright)$ a right pre-Lie algebra.  By the Oudom--Guin theorem, this determines a connected graded Hopf algebra on the symmetric algebra $S(V)\cong\ZZ[\Forest(\PTree)]$, with coproduct $\Delta(t)=t\otimes 1+1\otimes t$ on primitive generators and antipode given by the standard connected-graded recursion (Corollary~\ref{cor:hopf}).  The commutator $[x,y]=x\triangleright y-y\triangleright x$ defines a Lie bracket on $V$ whose bracket measures the directional asymmetry of proof substitution (Corollary~\ref{cor:lie}).

\noindent\textbf{Lean formalization.}
The main results have been formalized in Lean~4 using Mathlib.
The central theorem \texttt{classLevel\_preLie\_identity} is machine-verified
with no \texttt{sorry} placeholders in the live proof chain.

\medskip

The paper is organized as follows. Section~2 defines the primitive consequence
relation and the calculus $\NMMS(\mszero)$. Section~3 develops proof trees as
rooted ordered objects. Section~4 introduces matching-leaf grafting. Section~5
analyses the two-step graft geometry underlying the associator. Section~6
proves the pre-Lie identity. Section~7 develops the Oudom--Guin Hopf algebra.
Section~8 discusses the admissible-cut coproduct as complementary structure.
Section~9 records theorem targets and the structural-rules programme.
...
\begin{center}
\begin{tikzpicture}[
  nd/.style={circle,fill=black,inner sep=1.45pt},
  ed/.style={thick},
...
\begin{scope}[shift={(-5.8,0)}]
  \node[nd] (r) at (0,0) {};
  \node[lab,below=3mm of r] {one vertex\\base};
\end{scope}
...
\begin{scope}[shift={(-3.4,0)}]
  ...
  \node[nd] (a) at (0,1.25) {};
  \draw[ed] (r)--(a);
  \node[lab,below=3mm of r] {unary rule};
\end{scope}

% binary node
\begin{scope}[shift={(-0.8,0)}]
  \node[nd] (r) at (0,0) {};
  \node[nd] (a) at (-0.55,1.25) {};
  \node[nd] (b) at (0.55,1.25) {};
  \draw[ed] (r)--(a); \draw[ed] (r)--(b);
  \node[lab,below=3mm of r] {binary rule};
\end{scope}

% unary chain of height two
\begin{scope}[shift={(2.1,0)}]
  \node[nd] (r) at (0,0) {};
  \node[nd] (a) at (0,1.15) {};
  \node[nd] (b) at (0,2.3) {};
  \draw[ed] (r)--(a); \draw[ed] (a)--(b);
  \node[lab,below=3mm of r] {height \(2\)\\unary chain};
\end{scope}

% mixed height two
\begin{scope}[shift={(5.0,0)}]
  \node[nd] (r) at (0,0) {};
  \node[nd] (a) at (-0.7,1.15) {};
  \node[nd] (b) at (0.7,1.15) {};
  \node[nd] (c) at (-1.05,2.3) {};
  \node[nd] (d) at (-0.35,2.3) {};
  \draw[ed] (r)--(a); \draw[ed] (r)--(b);
  \draw[ed] (a)--(c); \draw[ed] (a)--(d);
  \node[lab,below=3mm of r] {height \(2\)\\mixed};
\end{scope}
\end{tikzpicture}
\end{center}
...
\node at (4.7,3.3) {$\operatorname{gr}_a(u,t)$};
...
\begin{remark}[What is proved here, and what remains separate]
The pre-Lie theorem supplies a complete input to the standard Oudom--Guin
machine, so the Hopf algebra on \(S(V)\) follows by an established theorem of
pre-Lie theory.  This does not by itself prove that the admissible-cut
coproduct below descends through the same proof-theoretic quotient, nor that
the cut coproduct is the coproduct of the Oudom--Guin Hopf algebra.  Those are
additional comparison and descent problems.
\end{remark}

\section{Cut coproducts and the coalgebra problem}

There is nevertheless a natural cut coproduct on proof trees.  It is useful
both for comparison with Connes--Kreimer theory and for isolating the remaining
coalgebraic work.

\paragraph{Admissible cuts.}
...
to a leaf meets at most one cut address.  Such a cut determines:
\[
P_C(t)=\prod_{a\in C} t\downarrow_a,
\qquad
R_C(t)=\text{the tree obtained by replacing each }t\downarrow_a
\text{ by a leaf}.
\]
Here \(P_C(t)\) is a forest of pruned subtrees, while \(R_C(t)\) is the
remainder or trunk.

\paragraph{The cut sum.}
The associated cut coproduct datum is
\[
\Delta_{\mathrm{cut}}(t)
=
...
P_C(t)\otimes R_C(t),
\]
with the empty cut contributing \(1\otimes t\).  Depending on convention, the
root cut contributes \(t\otimes 1\), or this term is adjoined separately.
...
\section{Proof-theoretic meaning: theorem targets and roadmap}

The previous sections isolate the part of the construction that is currently
mathematically solid: proof trees carry a constrained grafting product, the
two-step graft geometry gives a right pre-Lie identity, and the standard
Oudom--Guin theorem then supplies a Hopf algebra on the symmetric algebra of
proof trees.  The purpose of this section is different.  It records the
larger theorem targets suggested by the construction.  These statements are
not all proved in the present paper; they are included as a guide to the
proof-theoretic interpretation of the Hopf operations and to the next formal
steps.
...
There are two Hopf-algebraic routes in play.  The Oudom--Guin route is
already available from the pre-Lie theorem: proof trees are primitive
generators and the coproduct is the primitive-generator coproduct on forests.
The cut route is more proof-decompositional: the coproduct sums over
admissible cuts of a proof tree.  The following targets explain what each
operation should mean proof-theoretically.
