...
Structural rules such as Weakening and Contraction have of course been studied extensively in substructural logics such as linear logic, which controls duplication of hypotheses but also possesses the Cut Elimination property which is desirable from the point of view of computational content. NMMS is a very different setting -- here the sequent calculus is equipped on a primitive {base} consequence relation $\mszero$, which serves to encode non-logical patterns of defeasible \emph{material inference} of the sort studied by Brandom and collaborators in the \emph{inferentialist} analysis of meaning in natural language. 

\vspace{0.2cm}

In general, nonmonotonic consequence relations fail to admit the Cut rule:

This is reflected in the fact that NMMS does not admit a \emph{categorical semantics} where Cut is represented by composition of morphisms of some suitable category, as is the case of linear logic (*-autonomous or symmetric monoidal closed categories), intuitionistic logic (cartesian closed categories), or classical logic (control categories). Here we show that NMMS admits a non-trivial interpretation in a different algebraic structure, and specifically a Hopf algebra defined over the set of labelled and decorated rooted trees which are just the proof trees of the sequent calculus.
